				...
				echo '<ul id="'. $this->id .'_sortable">'; ?>
					...
					foreach ( $values as $val ) :
						...
						if ( $label ) : ?>
							<li data-value="<?php echo esc_attr( $val ); ?>" class="themesflat-sortable-li">
								...
								<span class="themesflat-hide-sortee fa icon-zeng-toggle-on"></span>
							</li>
						<?php
						endif;						
						unset( $choices[$val] );					
					...
					foreach ( $choices as $val => $label ) { ?>
						<li data-value="<?php echo esc_attr( $val ); ?>" class="themesflat-sortable-li themesflat-hide">
							<?php echo esc_html( $label ); ?>
							<span class="themesflat-hide-sortee fa icon-zeng-toggle-on icon-zeng-toggle-off"></span>
						</li>
					<?php } ?>
				</ul>
			</div>
				...
				jQuery(document).ready( function($) {
					...
					sortableUl.find( 'li' ).each( function() {
						$( this ).find( '.themesflat-hide-sortee' ).on('click', function() {
							$( this ).toggleClass( 'icon-zeng-toggle-off' ).parents( 'li:eq(0)' ).toggleClass( 'themesflat-hide' );
						} );
					});					
